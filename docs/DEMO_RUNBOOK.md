...
loads starter Shared Pods. Keep this as the answer to "where does the user's
...
Frame this carefully: a Shared Pod is a portable memory/context pod, not
necessarily an industry expert and not a replacement for professional advice.
It may carry an explicit task lens, checklist, examples, or principles. The
strongest demo example is `senior-review`: it is concrete, low-risk, and shows
how someone can hand you a review lens as an inspectable file.

Show the unrelated marathon task returning zero Shared Pod context. This is the
proof that Memory Pod is dynamic retrieval, not a static mega-prompt.
